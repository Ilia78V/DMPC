%% Two-Agent Distributed MPC Test Using a Simple Single Integrator with Coupling
clear; clc;

%% (Optional) Add paths for YALMIP/OPTI if needed
if false
...
rho_init = 100;  % Initial penalty parameter

%% Coupling Parameter
coupling_gain = 0.5;  % Determines the influence of the neighbor's state
...
%% Define Dynamics
% Local dynamics: simple single integrator (discrete time)
%   x[k+1] = x[k] + dt * u[k]
f_i = @(x, u) u;

% Coupling dynamics: a term that drives the state toward the neighbor's state.
% This term is added to the local dynamics.
f_ij = @(x, u, x_neighbor, u_neighbor) coupling_gain * (x_neighbor - x);
...
% Wrap the cost functions with the agent-specific reference.
...
%% Create Neighbor Objects for Coupling (now with a nonzero f_ij)
